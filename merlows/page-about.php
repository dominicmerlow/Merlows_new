...
<main id="main-content">
    ...
    function ibdhh_get_style_string($prefix, $default_bg = '') {
    ...
    <!-- HERO SECTION -->
    <?php if (get_theme_mod('ibdhh_about_hero_show', true)) : 
        $hero_img    = get_theme_mod('ibdhh_about_hero_img', get_template_directory_uri() . '/assets/img/hcp_hero.png');
        $hero_bg_color = get_theme_mod('ibdhh_about_hero_bg_color');
        $hero_tag   = get_theme_mod('ibdhh_about_hero_tag', 'Our Story');
        $hero_title = get_theme_mod('ibdhh_about_hero_title', 'From Pharma to <span class="highlight">Healthcare</span>');
        $hero_sub   = get_theme_mod('ibdhh_about_hero_sub', 'A Natural Evolution in Gastrointestinal Care');
        $hero_desc  = get_theme_mod('ibdhh_about_hero_desc', 'IBD Health Hub bridges the worlds of pharmaceutical science and patient-centred nutrition, delivering evidence-based medical food solutions for life with IBD.');
        
        $styles = ibdhh_get_style_string('ibdhh_about_hero');
    ...
    <!-- ORIGIN SECTION -->
    <?php if (get_theme_mod('ibdhh_about_origin_show', true)) : 
        $origin_tag   = get_theme_mod('ibdhh_about_origin_tag', 'From Pharma to Healthcare');
        $origin_title = get_theme_mod('ibdhh_about_origin_title', 'The IBD Health Hub Evolution');
        $origin_sub   = get_theme_mod('ibdhh_about_origin_sub', 'A Natural Evolution in Gastrointestinal Care');
        $styles = ibdhh_get_style_string('ibdhh_about_origin', '#fff');
    ?>
    <section id="our-story" class="section-padding" style="<?php echo $styles['section']; ?>">
        <div class="container">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:32px;margin-bottom:60px;">
                ...
                for($i=1; $i<=3; $i++):
                    $p_title = get_theme_mod("ibdhh_about_p{$i}_title", $pillar_defaults[$i][0]);
                    $p_desc = get_theme_mod("ibdhh_about_p{$i}_desc", $pillar_defaults[$i][1]);
                ?>
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:32px;text-align:center;padding-top:40px;border-top:1px solid rgba(0,0,0,0.1);">
                ...
                for($i=1; $i<=3; $i++):
                    $s_num = get_theme_mod("ibdhh_about_stat{$i}_num", $stat_defaults[$i][0]);
                    $s_lbl = get_theme_mod("ibdhh_about_stat{$i}_label", $stat_defaults[$i][1]);
                ?>
                <div>
    ...
    <!-- MISSION & VALUES -->
    <?php if (get_theme_mod('ibdhh_about_mission_show', true)) : 
        $mission_tag = get_theme_mod('ibdhh_about_mission_tag', 'Our Mission');
        $mission_title = get_theme_mod('ibdhh_about_mission_title', 'Bridging Science & <span class="highlight">Patient Wellbeing</span>');
        $mission_desc = get_theme_mod('ibdhh_about_mission_desc', 'At IBD Health Hub, our mission is to empower patients living with chronic gastrointestinal conditions by making world-class clinical nutrition science accessible, actionable, and personal.');
        $styles = ibdhh_get_style_string('ibdhh_about_mission', '#f8f9fa');
    ?>
    <section id="mission" class="section-padding" style="<?php echo $styles['section']; ?>">
        <div class="container">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:32px;">
                ...
                for($i=1; $i<=4; $i++):
                    $v_title = get_theme_mod("ibdhh_about_val{$i}_title", $val_defaults[$i][0]);
                    $v_desc = get_theme_mod("ibdhh_about_val{$i}_desc", $val_defaults[$i][1]);
                ?>
    ...
    <!-- EPAVANCE SPOTLIGHT -->
    <?php if (get_theme_mod('ibdhh_about_product_show', true)) : 
        $prod_tag = get_theme_mod('ibdhh_about_prod_tag', 'Our Flagship Product');
        $prod_title = get_theme_mod('ibdhh_about_prod_title', 'Introducing EPAVANCE');
        $prod_desc = get_theme_mod('ibdhh_about_prod_desc', 'EPAVANCE is a pharma-grade Omega-3 medical food especially formulated for patients with Inflammatory Bowel Disease. Unlike generic supplements, EPAVANCE is developed under the same rigorous manufacturing standards applied to licensed medicines.');
        $prod_btn = get_theme_mod('ibdhh_about_prod_btn', 'Learn More About EPAVANCE');
        $prod_url = get_theme_mod('ibdhh_about_prod_url', '#');
        $styles = ibdhh_get_style_string('ibdhh_about_product', '#fff');
    ?>
        ...
        <div class="container">
            <div style="display:flex;flex-wrap:wrap;gap:40px;align-items:center;">
                ...
                <div style="flex:1;min-width:300px;display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:20px;">
                    ...
                    for($i=1; $i<=4; $i++):
                        $f_title = get_theme_mod("ibdhh_about_feat{$i}_title", $feat_defaults[$i][0]);
                        $f_desc = get_theme_mod("ibdhh_about_feat{$i}_desc", $feat_defaults[$i][1]);
                    ?>
    ...
    <!-- PLATFORM SECTION -->
    <?php if (get_theme_mod('ibdhh_about_platform_show', true)) : 
        $plat_tag = get_theme_mod('ibdhh_about_plat_tag', 'The Digital Layer');
        $plat_title = get_theme_mod('ibdhh_about_plat_title', 'The IBD Health Hub Platform');
        $plat_desc = get_theme_mod('ibdhh_about_plat_desc', 'Beyond our medical food products, IBD Health Hub is building a world-class digital health hub - combining clinical-grade content, AI-powered tools, and a vibrant community for patients and healthcare professionals.');
        $styles = ibdhh_get_style_string('ibdhh_about_platform', '#142846');
    ?>
    <section class="section-padding" style="<?php echo $styles['section']; ?> color:white;">
        <div class="container">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:24px;">
                ...
                for($i=1; $i<=6; $i++):
                    $pl_title = get_theme_mod("ibdhh_about_plat{$i}_title", $plat_defaults[$i][0]);
                    $pl_desc = get_theme_mod("ibdhh_about_plat{$i}_desc", $plat_defaults[$i][1]);
                ?>
    ...
    <?php for ($p=1; $p<=2; $p++) : 
        ...
        if (get_theme_mod($prefix . '_show', true)) :
            ...
            $styles = ibdhh_get_style_string($prefix, '#fff');
    ...
    <!-- CTA STRIP -->
    <?php if (get_theme_mod('ibdhh_about_cta_show', true)) : 
        $cta_title = get_theme_mod('ibdhh_about_cta_title', 'Join the IBD Health Hub Community');
        $cta_desc = get_theme_mod('ibdhh_about_cta_desc', "Whether you're a patient managing IBD, a clinician advancing your practice, or a researcher exploring gut health - there's a place for you at IBD Health Hub.");
        $cta_btn1_lbl = get_theme_mod('ibdhh_about_cta_btn1_label', "I'm a Patient");
        $cta_btn1_url = get_theme_mod('ibdhh_about_cta_btn1_url', '/patients/');
        $cta_btn2_lbl = get_theme_mod('ibdhh_about_cta_btn2_label', "I'm a Healthcare Professional");
        $cta_btn2_url = get_theme_mod('ibdhh_about_cta_btn2_url', '/healthcare-professionals/');
        $styles = ibdhh_get_style_string('ibdhh_about_cta', '#EB5A33');
    ?>
