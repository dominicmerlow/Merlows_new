...
async function searchSemanticScholar(query, maxResults = 20) {
  ...
  const res = await fetchWithRetry(url, { headers: { 'User-Agent': 'Merlows/1.0' } });
...
async function searchReliefWeb(query, maxResults = 20) {
  ...
  const res = await fetchWithRetry(`https://api.reliefweb.int/v2/reports?${params}`);
...
async function searchCongress(query, maxResults = 20) {
  ...
  const res = await fetchWithRetry(`https://api.congress.gov/v3/committee-report?${params}`);
...
async function searchWorldBank(query, maxResults = 20) {
  ...
  const res = await fetchWithRetry(`https://search.worldbank.org/api/v3/wds?${params}`);
