...
async function generateArticle(item, rule, fetchFn = fetch) {
  ...
  // Fetch server-side master prompt (saved via Settings → Global Context)
  const masterPrompt = ((await kv.get('settings:masterPrompt')) ?? '').trim();
  const masterPromptBlock = masterPrompt
    ? `[GLOBAL CONTEXT — always incorporate this into your response]\n${masterPrompt}\n\n[TASK]\n`
    : '';
  const fullPrompt = `${masterPromptBlock}${basePrompt}\n\nSOURCE FILE: ${item.title}\n\nSOURCE MATERIAL:\n${item.rawText || '(source text not available — generate based on the filename/title only)'}`;
