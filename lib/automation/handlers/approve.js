...
import { writeSheetPublishNote } from '../../sources/google-sheets.js';
...
export default async function handler(req, res) {
  ...
  if (action === 'approve') {
    ...
    // Promote content status to 'approved' so the publish endpoint accepts it
    const content = await kv.get(`content:${job.contentId}`);
    if (content && content.status === 'draft') {
      await kv.set(`content:${job.contentId}`, { ...content, status: 'approved', updatedAt: now });
    }
    ...
    if (rule?.publish?.wordpress) {
      try {
        ...
        if (publishRes.ok) {
          ...
          // Write live WP URL back to the originating Google Sheet row (non-fatal)
          const publishedContent = await kv.get(`content:${job.contentId}`);
          if (publishedContent?._sheetMeta && publishData?.wpPostUrl) {
            await writeSheetPublishNote(publishedContent._sheetMeta, publishData.wpPostUrl);
          }
